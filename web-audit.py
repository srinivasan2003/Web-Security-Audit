...
def helper():
        ...
        print("------------")
        print("\t./web-audit.py example.com: Scans the domain example.com.")
        print("\t./web-audit.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./web-audit.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./web-audit.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits web-audit")
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/web-audit_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/web-audit_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/web-audit_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/web-audit_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/web-audit_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o web-audit_temp_wapiti"],
...
analyze_urls_from_input(args_namespace.target, "security-header.txt", 0.8)
...
if args_namespace.help or (not args_namespace.update \
    ...
    os.system('rm /tmp/web-audit* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"web-audit was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by web-audit."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. web-audit will still perform the rest of the tests. Install these tools to fully utilize the functionality of web-audit."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/web-audit_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    debuglog = "web-audit.dbg.%s.%s" % (target, date)
    vulreport = "web-audit.vul.%s.%s" % (target, date)
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/web-audit_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/web-audit_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/web-audit_te* > /dev/null 2>&1') # Clearing previous scan files
